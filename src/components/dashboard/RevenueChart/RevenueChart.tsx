...
const CustomDot = (props: DotProps & { payload?: PayloadType }) => {
        ...
        <ChartContainer config={chartConfig}>
          <ResponsiveContainer width="100%" height={400}>
            ...
            >
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(215, 25%, 25%)" horizontal vertical={false} />
              ...
              <ChartTooltip
                ...
                content={({ active, payload, label }) => {
                  if (active && payload && payload.length) {
                    ...
                    if (revenueData && label === "Jun") {
                      return (
                        <div className="bg-slate-800 border border-slate-700 rounded-lg p-3 shadow-lg">
                          <div className="text-slate-200 font-medium">${revenueData.value}</div>
                          <div className="text-emerald-400 text-sm flex items-center gap-1">
                            <TrendingUp className="h-3 w-3" />
                            +12.4%
                          </div>
                          <div className="text-slate-400 text-xs mt-1">Jun 18, 2025</div>
                        </div>
                      );
                    }
                  }
